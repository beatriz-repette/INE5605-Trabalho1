...
class ControladorGato():
    ...
    def listar_gatos(self):
        if self.gatos == []:
            self.__tela_gato.mensagem_sem_gatos()
        else:
            dados = []
            for a in self.gatos:
                nomes_vacinas = ''
            ...
            self.__tela_gato.mostrar_gato(dados)
    ...
    def adicionar_vacina(self):
        if self.gatos == []:
            ...
            try:
                ...
                if chip == '*':
                    self.__tela_gato.mensagem_operacao_cancelada()
                else:
                    gato = self.gato_por_chip(chip)
                    if gato != 'Gato nao se encontra no sistema.':
                        dados = self.__tela_gato.pegar_vacina(chip)
                        if dados == 0:
                            self.__tela_gato.mensagem_operacao_cancelada()
                        else:
                            gato.add_vacina(
                ...
                self.__tela_gato.mensagem_erro_vacina()
    ...
    def alterar_gato(self):
        if self.gatos == []:
            ...
            gato = self.gato_por_chip(chip)
            if gato != 'Gato nao se encontra no sistema.':
                dados = self.__tela_gato.pegar_dados_gato()
                if dados != 0:
                    ...
                    if dados['chip'] != '*':
                        ...
                        if (self.__controlador_principal.animal_por_chip(
                            ...
                            doacao = self.__controlador_principal.controladorDoacao
                            ...
                            self.__tela_gato.mostrar_mensagem('Ja existe um animal com esse chip.')
                    ...
                    self.__tela_gato.mensagem_operacao_concluida()
                else:
                    self.__tela_gato.mensagem_operacao_cancelada()
            else:
                self.__tela_gato.mostrar_mensagem(gato)
