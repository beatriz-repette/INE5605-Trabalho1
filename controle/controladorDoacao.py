...
class ControladorDoacao():
    ...
    def incluir_doacao(self):
        try:
            ...
            self.__tela_doacao.mensagem('Ja existe um animal com esse chip no sistema.')
        except:
            pass
    ...
    def excluir_doacao(self):
        try:
            doacao = self.__tela_doacao.seleciona_doacao(len(self.__doacoes))
            if doacao == '*':
                    self.__tela_doacao.mensagem_operacao_cancelada()
                    raise RetornarException
    ...
    def alterar_doacao(self):
        try:
            ...
            if novos_dados_doacao["chip"] != '*':
                animal = self.__controlador_principal.controladorAnimal.animal_por_chip(doacao.animal)
                if self.__controlador_principal.controladorAnimal.animal_por_chip(novos_dados_doacao['chip']) == 'Animal não se encontra no sistema.':
